    ...
    public String callback(String code, String state, Model model, HttpServletResponse response) {
        ...
        if (null != SecurityContextHolder.getContext().getAuthentication()) {
            ...
            if (null != principal && principal instanceof CommonUserDetails) {
                ...
                if (null == commonUserDetails || null == commonUserDetails.getBaseUser().getId() || org.apache.commons.lang.StringUtils.isBlank(commonUserDetails.getUsername())) {
                    return "503";
                }
                ...
                ResultMessage<String> rsp = authClient.bindSocialAccount(commonUserDetails.getBaseUser().getId(), gitHubLoginInfo.get("login"), OAuthTypes.GITHUB.getCode());
