...
public class RbacService {
    ...
    public boolean hasPermission(HttpServletRequest request, Authentication authentication) {
        ...
        if (null != principal && principal instanceof CommonUserDetails) {
            ...
            if ("admin".equals(commonUserDetails.getUsername())) {
                return true;
            }
            if (CollectionUtils.isEmpty(commonUserDetails.getRoleIds())) {
                return false;
            }
            //鉴权时候，若能根据当前平台过滤掉一部分非当前平台的角色url
            Set<String> urls = getUrlsByRoles(commonUserDetails.getRoleIds(), getPlatformId(request));
            for (String url : urls) {
