...
export class AuthComponent implements OnInit {
  ...
  public getTreeList(auth?: AuthServiceNs.SysAuthsItemModel) {
    ...
    this.loading = true;
    this.authService.getAuthList(id, this.filters.platformId).subscribe((resData: AuthServiceNs.UfastHttpAnyResModel) => {
      this.loading = false;
      if (resData.status !== 0) {
      ...
      if (auth) {
        auth.children = resData.data;
        auth.expand = true;
      } else {
        this.authDataList = resData.data;
        this.topNode = {};
        this.topNode.id = 0;
  ...
  public addOrEditAuth(type: number, target?: AuthServiceNs.SysAuthsItemModel | any) {
    ...
    if (type === 1) {
      ...
      this.targetAuth.platformId = this.filters.platformId;
    } else {
